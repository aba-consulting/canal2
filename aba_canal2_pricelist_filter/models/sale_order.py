...
class SaleOrder(models.Model):
    ...
    def _compute_is_canal_2_type(self):
        for order in self:
            order.is_canal_2_type = bool(
                order.type_id and order.type_id.name and "Canal 2" in order.type_id.name
            )
    ...
    def _onchange_type_id_clear_pricelist_canal2(self):
        ...
        if self.type_id and self.pricelist_id:
            is_canal_2_type = self.type_id.name and "Canal 2" in self.type_id.name
            is_canal_2_pricelist = self.pricelist_id.name and "Canal 2" in self.pricelist_id.name
